...
# testprobe<-function(row, pheno){
# 	model<-lm(row ~ pheno$Age + factor(pheno$Sex) + factor(pheno$Chip))
# 	return(summary(model)$coefficients["pheno$Age",c(1,2,4)],
# 	       summary(model)$coefficients["factor(pheno$Sex)M",c(1,2,4)])
# }
# 
# ###Run EWAS using foreach() and %dopar% to tell R to run the analysis is parallel
# res<-foreach(i=1:nrow(betas), .combine=rbind) %dopar% 
#   {
# 		testprobe(betas[i,], pheno)
# 	}
...
write.csv(res, "res_f__chip_included_manifest.csv")
```
...
# res <- read.csv("res_f__chip_included_manifest.csv", stringsAsFactors = F, header=T, row.names = 1)
par(mfrow = c(2,2))
#pdf('QQplot of sex.pdf')
qq(res$Sex_Beta)
#dev.off()
...
#pdf('manhattan for sex differences.pdf')
...
vec<-rep(NA, length(as.character(pheno$Sex)))
vec[which(as.character(pheno$Sex)=="M")]<-"light blue"
vec[which(as.character(pheno$Sex)=="F")]<-"dark blue"
my_palette11 <- colorRampPalette(c("yellow", "red4" ))(n = 1000)


par(mfrow = c(2,1))
#pdf('heatmap of top varying Probes.pdf')
heatmap.2(betas[order(sigma, decreasing = TRUE)[1:500],], trace = "none", labCol = rownames(pheno), dendrogram = "column", labRow = "", mar = c(8,1), density.info = "none", scale = "none", ColSideColors = vec)
legend("topright",col = c("light blue", "dark blue"), pch = 16, c("Male", "Female"), horiz = TRUE, cex=0.8)
#dev.off()
#pdf('heatmap of top 5000 Significant Probes.pdf')
heatmap.2(betas_r, trace = "none", labCol = pheno$Sex, dendrogram = "column", labRow = "", mar = c(5,1), density.info = "none", scale = "none", ColSideColors =vec)
legend("topright",col = c("light blue", "dark blue"), pch = 16, c("Male", "Female"), horiz = TRUE, cex=0.8)
#dev.off()
