...
## Load the data for cerebellum analysis
...
bb <- read.csv("/mnt/data1/BrainData/Results/Age_Sex/CEREB/LM_AgeSq_Age_Sex_Cov_Chip.csv", header= T, row.names = 1)
...
Doing this for both age and sex will save time than doing them individually
...
Pull out the probes that are both present in the UK and Indian samples

``` {r matchingsig probes}
...
## Compare the probes between UK and India
Load the India raw beta data

```{r load India data}
load("/mnt/data1/NIMHANS/Down_stream_analysis/NIMHAM_CER_Normalised_snpsremoved.rdat") 
pheno_india <- pheno 
betas_india <- betas
...
Load the UK raw data but filter for the cerebellum brain region
```{r load UK data}
...
This will help us know more information about the samples such as age, sex and whether the samples are from the UK or India.
```{r metadata}
...
identical(rownames(betas_ui), rownames(cells))
...
pheatmap(betas_ui, scale = 'row',
         ...
         show_rownames = F,
         show_colnames = F,
         main="Top significant age probes between UK and Indian samples (cereb)")

```

#Part 2: Sex
Th effect of sex on cerebellum methylation will be compared in both datasets

```{r comparison of sex}
#edit the res_sex and bb_sex
...
pheatmap(betas_ui2, scale = 'row',
         ...
         show_rownames = F,
         show_colnames = F,
         main="Top significant sex probes between UK and Indian samples (cereb)")
...
ggscatter(bb_res_age, x = 'Age_Beta', y = 'Age.Coeff',
  ...
  labs(title = "Correlation between UK and Indian significant sites on age (Cer)",
       x = 'India', y = 'UK')
...
ggscatter(bb_res_sex, x = 'Sex_Beta', y = 'Sex.Coeff',
          ...
          conf.int = T) +
  stat_cor(method = 'pearson') +
  labs(title = "Correlation between UK and Indian significant sites on sex (Cer)",
       x = 'India', y = 'UK')
