...
``` {r GO term analysis Age, warning = F}
#India frontal (Age)
library(missMethyl)
library(IlluminaHumanMethylation450kanno.ilmn12.hg19)
library(limma)
ann <- getAnnotation(IlluminaHumanMethylation450kanno.ilmn12.hg19)
#res2 <- res #run this once then comment it out
#AD
res <- res2
index<-which(res$Age_P<=0.001)# this can be set depending on the p-value disribution of your dataset we normally set it to <p=1x10-5
background<-rownames(res) #take the original rownames of res
rownames(res)[index]->test #take the rownames of res which have AD_P less than 0.001
...
gst2 <- gst[1:5,] #seelct top 5 terms
...
pdf('GO India Cereb Age.pdf') 
par(mar =c(15,15,15,30))
ggplot(gst2, aes(x =Term, y = N)) +
  geom_bar(fill = 'royalblue', stat = 'identity') +
  coord_flip() + 
  labs(x = "", y = "", title = 'Top 5 significant GO terms for age in India frontal samples') +
...
#UK frontal (Age)
index<-which(bb$Age.P<=0.001)# this can be set depending on the p-value disribution of your dataset we normally set it to <p=1x10-5
background<-rownames(bb) #take the original rownames of res
rownames(bb)[index]->test #take the rownames of res which have AD_P less than 0.001
...
gst2$Term<- factor(gst2$Term, levels = gst2$Term) #lock it as factors so it does not go in alphabetical order
pdf('GO UK Cereb Age.pdf') 
par(mar =c(15,15,15,30))
ggplot(gst2, aes(x =Term, y = N)) +
  geom_bar(fill = 'royalblue', stat = 'identity') +
  coord_flip() + 
  labs(x = "", y = "", title = 'Top 5 significant GO terms for age in UK frontal samples') +
  geom_text(data=gst2,aes(x=Term,y=N,label=P.DE),position = position_stack(vjust = 0.5)) + #adds the p.val text to GO terms
  theme_classic()
dev.off()
